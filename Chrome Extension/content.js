(async function() {
    ...
    // ======= 0. 设置项管理 (Chrome 扩展: chrome.storage 持久化) =======
    const defaults = {
    ...
    let showSidebar = stored.mwca_show_sidebar;
    let showPublishCard = stored.mwca_show_publish_card;
    let useHammerLogo = stored.mwca_use_hammer_logo;
...
._mar1_1n75r_2 button.woo-button-main {
    ...
    function injectNewBackTop() {
        ...
        if (!document.body) return; // document_start 阶段 body 可能尚未生成, 由观察器稍后再次调用
...
document.querySelectorAll('.woo-font.woo-font--comment._commentIcon_198pe_122').forEach(iconEl => {
    ...
    // 接收设置面板(popup)的刷新消息: 切换功能开关后自动刷新当前微博页
    chrome.runtime.onMessage.addListener((message) => {
        if (message && message.action === 'mwca_reload') {
            location.reload();
        }
    });
})();